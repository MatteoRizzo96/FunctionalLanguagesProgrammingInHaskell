{-- Ex 5
    ...
    (2) Homomorphism law: pure (g x) = pure g <*> pure x
        it means that the pure function preserves function application.
        We have two variables "g" and "x".
        - g is a "simple" function (hence is not an "applicative" function) which is the argument of pure function, so its type
          must be g :: a -> b, and :t pure g :: f (a -> b)
        - even x is a "simple" variables, so its type must be x :: a, and :t pure (g x) :: f b . 
          Notice that "x" is the argument of "g", hence "x" and the input of "g" must have the same type.
